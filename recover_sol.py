...
def recover_sol(
    ...
    print(f"Sending {sender_balance / 1e9} SOL from {sender_keypair.pubkey()} to {receiver_keypair.pubkey()}")
...
def main():
    ...
    receiver_priv_base58_str = "<RECEIVER_PRIVATE_KEY_BASE58>"
   
    result = recover_sol(
        rpc_url=rpc_url,
        sender_priv_base58_str=sender_priv_base58_str,
        receiver_priv_base58_str=receiver_priv_base58_str,
    )
    if result is True:
        print("SOL recovery successful!")
    elif result is False:
        print("SOL recovery failed.")
    else:
        print("An error occurred during the SOL recovery process.")
